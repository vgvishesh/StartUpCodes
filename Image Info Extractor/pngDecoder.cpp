// Image information gets stored in ImageInfo structure....
...
void main()
{
	...
	png_uint_32 imgWidth =  Image.width = png_get_image_width(pngPtr, pngInfo);
	png_uint_32 imgHeight = Image.height = png_get_image_height(pngPtr, pngInfo);

    //bits per CHANNEL! note: not per pixel!
	png_uint_32 bitdepth   = Image.colorDepth= png_get_bit_depth(pngPtr, pngInfo);
    //Number of channels
	png_uint_32 channels   = Image.channels = png_get_channels(pngPtr, pngInfo);
    //Color type. (RGB, RGBA, Luminance, luminance alpha... palette... etc)
	png_uint_32 color_type = Image.colorType = png_get_color_type(pngPtr, pngInfo);
